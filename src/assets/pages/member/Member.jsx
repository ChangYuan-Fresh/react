...
function Member() {
  ...
  const handleCopyCode = () => {
    navigator.clipboard
      .writeText(discountCode) // 複製折扣碼到剪貼簿
      .then(() => {
        dispatch(
          createAsyncMessage({
            text: '折扣碼已複製！',
        ...
        <div className="row my-lg-7 my-0 g-5">
          <div className="col-lg-3">
            <div className="card border-0 d-none d-lg-block">
              <div
                className="card-body bg-secondary-200 rounded rounded-3 mb-3 px-5 pb-5 pt-11 position-relative w-100"
                style={{ marginTop: '132px' }}
              >
                <img
                  src="images/cat.jpg"
                  className="position-absolute top-0 start-50 translate-middle object-fit-cover"
                  alt="member-pic"
                  style={{
                    borderRadius: '12px',
                    width: '200px',
              ...
              <div className="card-body bg-secondary-200 rounded rounded-3 mb-3">
                <Link
                  to="/changePassword"
                  className="btn w-100 fs-6 py-3 text-start"
                >
                  修改密碼
                </Link>
                <Link className="btn w-100 fs-6 py-3 text-start">
                  收件地址管理
                </Link>
                <button
                  type="button"
                  className="btn w-100 fs-6 py-3 d-flex justify-content-between align-items-center"
                  onClick={handleCouponExtend}
                >
                  我的優惠券
              ...
              <div className="card-body bg-secondary-200 rounded rounded-3">
                <Link
                  ...
                  className="mt-auto ms-auto fs-5 fs-lg-4 link-primary mb-3"
                >
                  登出
                ...
                <Link
                  to="/"
                  className="mt-auto ms-auto fs-5 fs-lg-4 link-primary"
                >
                  回到首頁
                </Link>
              </div>
              ...
              <div className="card-body bg-secondary-200 rounded rounded-3 mb-3 px-5 pt-4 w-100 ">
                <div className="d-flex">
                  ...
                  <div>
                    ...
                    <p className="text-gray mb-5 fs-7 en-font">
                      asd456@gmail.com
                    </p>
                  </div>
                </div>
                ...
                >
                  {textExtend ? (
                    <>
                      ...
                      <Link className="text-accent text-start pb-3 w-100">
                        修改會員資料
                      </Link>
                      <div className="card-footer bg-secondary-200 px-0">
                        <Link
                          to="/changePassword"
                          className="btn w-100 fs-6 text-start pt-3 pb-2  border-0"
                        >
                          修改密碼
                        </Link>
                        <Link className="btn w-100 fs-6 text-start pt-0 pb-2 border-0">
                          收件地址管理
                        </Link>
                        <button
                          type="button"
                          className="btn w-100 fs-6 pt-0 pb-2 d-flex justify-content-between align-items-center"
                          onClick={handleCouponExtend}
                        >
                          我的優惠券
                          <span className="material-symbols-outlined">
                ...
                {orderState.map(state => {
                  return (
                    <li className="nav-item" key={state}>
                      <button
                        className={`nav-link bg-white border-0 border-3 py-4 border-bottom px-9 ${selectState === state ? 'border-primary text-primary' : ' border-white text-dark'}`}
                        type="button"
                  ...
                  <table className="table border-primary">
                    <thead>
                      <tr className="table-borderless">
                        <th
                          scope="col"
                          className="bg-secondary-200 text-gray p-3 w-12"
                          width="15%"
                        >
                          訂單編號
                        </th>
                        ...
                        <th
                          scope="col"
                          className="bg-secondary-200 text-gray p-3"
                          width="40%"
                        >
                          訂購商品內容
                        </th>
                      ...
                      {filterOrders.length === 0 ? (
                        <tr>
                          <th colSpan="6" className="text-center border-0">
                            <img
                              src="images/Illustration/Frame.png"
                              alt="empty"
                        ...
                        filterOrders.map(order => {
                          return (
                            <tr key={order.id} className="py-5">
                              ...
                              <td className="py-5">
                                <FormatDate timestamp={order.create_at} />
                              </td>
                              {order.products &&
                                        ...
                                        <div className="col-9 px-0">
                                          <div className="d-flex justify-content-between mb-2 w-100">
                                            ...
                                            {order.is_paid ? (
                                              <button
                                                type="button"
                                                className="btn text-accent p-0 border-0"
      ...
      <main className="d-lg-none">
        ...
        <ul className="nav nav-tabs justify-content-center w-100 d-flex justify-content-between border-0">
          {orderState.map(state => {
            return (
              <li className="nav-item" key={state}>
                <button
                  className={`nav-link bg-white border-0 border-3 py-4 border-bottom px-4 ${selectState === state ? 'border-primary text-primary' : ' border-white text-dark'}`}
                  type="button"
                  onClick={() => setSelectState(state)}
                >
                  {state}
        ...
        {filterOrders.length === 0 ? (
          ...
          filterOrders.map(order => {
            ...
            return (
              <div
                key={order.id}
                className="card my-4 mx-3 border-0 p-4 bg-secondary-200"
              >
                <div
                  className={`card-title d-flex justify-content-between mb-0 ${isExpanded ? 'border-bottom pb-4' : ''}`}
                ...
                {isExpanded && (
                  <>
                          ...
                          >
                            <div className="d-flex">
                              <img
                                src={item.product?.imageUrl}
                                alt={item.id}
                                className="object-fit-cover rounded-1 me-3"
                    ...
                    <div className="d-flex align-items-center justify-content-end">
                      <button type="button" className="btn text-accent">
                        查看配送狀態
                      </button>
        ...
        <div className="card-body rounded rounded-3 d-lg-none d-flex justify-content-center align-items-center mx-3">
          <Link
            to="/"
            onClick={handleLogout}
            className="fs-5 link-primary py-3 me-5"
          >
            登出
          </Link>
          <Link to="/" className="fs-5 link-primary py-3">
            回到首頁
          </Link>
        </div>
      </main>
