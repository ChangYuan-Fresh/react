  ...
  const getStoryDetail = useCallback(async () => {
    ...
    try {
      const res = await axios.get(
        `${baseUrl}/v2/api/${apiPath}/article/${story_id}`
      );
      ...
      <main>
        ...
        <section>
          <div className="container pb-lg-11 pt-lg-7 py-md-7 py-4 d-flex flex-column flex-lg-row position-relative">
            <img
              src="images/Illustration/trees.png"
              alt="tree"
              className="deco-tree"
            />
            <article className="story-article bg-secondary-200 p-lg-7 p-5 rounded-3 mb-5 mb-lg-0">
              <div className="d-sm-flex justify-content-between align-items-baseline mb-lg-5 mb-3">
                ...
                <p className="text-gray fs-7 fw-normal">
                  {new Date(story.create_at).toLocaleDateString()}
                </p>
              </div>
              <p dangerouslySetInnerHTML={{ __html: story.description }}></p>
              <img
                src={story.image}
                className="img-fluid story-cauliflower-img"
              />
            ...
            <aside className="ms-lg-5 mb-8 mb-lg-5">
              <div
                className="list-group list-group-flush p-5 bg-secondary rounded-3"
                style={{ position: 'sticky', top: '130px' }}
              >
                <Link
                  className="story-cauliflower-btn list-group-item list-group-item-action bg-primary text-white text-center mb-3"
                  to="/Stories"
                >
                  產地故事
                </Link>
                {stories.map(item => {
                  return (
                    <Link
                      className={`story-cauliflower-btn list-group-item list-group-item-action mb-3 d-flex align-items-center ${item.id === story_id ? 'active' : ''}`}
                      key={item.id}
                      to={`/stories/${item.id}`}
                    >
