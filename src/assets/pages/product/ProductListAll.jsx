...
function ProductListAll() {
  ...
  const filterProducts = products
    ...
    .filter(
      product => product.title.toLowerCase().includes(searchQuery.toLowerCase()) // 根據搜尋框篩選
    )
  ...
  const handleCategoryChange = category => {
    setSelectCategory(prevCategory =>
      prevCategory === category ? '全部商品' : category
    );
  ...
  return (
    <>
      <div className="overflow-hidden container mb-5">
        ...
        <Swiper
          slidesPerView={3.3}
          className="nav nav-pills d-lg-none d-flex justify-content-between flex-nowrap text-nowrap allProduct-nav-pills"
          id="productTab"
          role="tablist"
        >
      ...
      <section className="bg-Tertiary container allProduct-container">
        <div className="row d-flex flex-lg-row flex-column-reverse justify-content-between">
          <div className="col-xl-2 col-lg-3 allProduct-side overflow-auto d-lg-block d-none">
            {/* <!-- 手風琴版選單列表 --> */}
            <section
              className="accordion d-lg-block d-none"
              id="accordionSidebar"
            >
              {categories.map(category => {
                return (
                  <div className="accordion-item" key={category}>
                    <h2 className="accordion-header" id="headingTwo">
                      <button
                        type="button"
                        className={`btn px-0 fw-bold fs-4 ${selectCategory === category ? 'text-primary collapsed' : 'text-dark'}`}
          ...
          <section className="col-xl-9 col-lg-8 allProduct-catalog">
            ...
            <form
              ...
              className="d-flex position-relative w-50 mb-5"
            >
              <input
            ...
            <div className="d-flex justify-content-between align-items-center">
              ...
              <div className="dropdown">
                <button
                  className="btn dropdown-toggle border-0 px-0 allProduct-catalog-sort"
                  type="button"
              ...
              {filterProducts.length > 0 ? (
                filterProducts.map(product => (
                  <div className="col-xl-4 col-lg-6 col-md-4 " key={product.id}>
                    <Link
                      className="card my-5 allProduct-catalog-card d-flex flex-md-column flex-row pb-4"
                      to={`/products/${product.category}/${product.id}`}
                      onClick={() => handleViewProduct(product)}
                    >
                      <img
                        src={product.imageUrl}
                        className="card-img-top allProduct-catalog-img"
                        alt={product.title}
                      />
                      <div className="card-body py-0 px-md-3 ps-5 pe-0">
                        <h5 className="card-title mt-md-3 mt-0 mb-md-4 mb-2 fs-5">
                          {product.title}
                        </h5>
