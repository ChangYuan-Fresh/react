  ...
  const getTotalProducts = useCallback(async () => {
    try {
      const res = await axios.get(
        `${baseUrl}/v2/api/${apiPath}/admin/products/all`
      );
      const allProducts = Object.values(res.data.products); // 取得所有商品陣列
      setTotalProducts(allProducts.length); // 計算全部商品數
      setTotalOnSale(
        allProducts.filter(product => product.is_enabled === 1).length
      ); // 上架中
      setTotalNoOnSale(
        allProducts.filter(product => product.is_enabled === 0).length
      ); // 未上架
      dispatch(
        createAsyncMessage({
          text: '取得所有商品數量成功',
        ...
        <div className="row">
          <div className="col-12">
            <div className="d-flex justify-content-between mb-6">
              ...
              <button
                type="button"
                className="btn btn-primary py-1 text-white f-6"
            ...
            <div className="bg-white  rounded-3 p-3">
              <ul className="nav nav-tabs" id="myTab" role="tablist">
                <li className="nav-item" role="presentation">
                  <button
                    className="nav-link products-nav-link border-0 active"
                    id="products-tab"
                    data-bs-toggle="tab"
                ...
                <li className="nav-item" role="presentation">
                  <button
                    className="nav-link products-nav-link border-0"
                    id="onSale-tab"
                    data-bs-toggle="tab"
                    data-bs-target="#onSale-tab-pane"
                    type="button"
                    role="tab"
                    aria-controls="onSale-tab-pane"
                    aria-selected="false"
                  >
                    上架中({totalOnSale})
                  </button>
                ...
                <li className="nav-item" role="presentation">
                  <button
                    ...
                    id="noOnSale-tab"
                    data-bs-toggle="tab"
                    data-bs-target="#noOnSale-tab-pane"
                    type="button"
                    role="tab"
                    aria-controls="noOnSale-tab-pane"
                    aria-selected="false"
                  >
                    未上架({totalNoOnSale})
                  </button>
              ...
              <form
                role="search"
                className="position-relative d-flex py-5"
                style={{ width: screenWidth > 767 ? '50%' : '100%' }}
              >
                <div className="dropdown position-absolute top-50 start-0 translate-middle-y z-2">
                  ...
                  <ul className="dropdown-menu py-0">
                    <li>
                      <Link className="dropdown-item fw-semibold">
                        全部商品
                      </Link>
                    </li>
                  </ul>
              ...
              <div className="tab-content " id="myTabContent">
                ...
                <div
                  className="tab-pane fade"
                  id="noOnSale-tab-pane"
                  role="tabpanel"
                  aria-labelledby="noOnSale-tab"
                  tabIndex="0"
                >
    ...
    return (
      <>
        ...
        <table className="table d-none d-sm-table">
          <thead>
            <tr>
              <th className="bg-secondary-200 text-gray text-center fs-7">
                編號
              </th>
              <th className="bg-secondary-200 text-gray fs-7">商品</th>
              <th className="bg-secondary-200 text-gray fs-7"></th>
              <th className="bg-secondary-200 text-gray fs-7 text-center">
                售價
              </th>
              <th className="bg-secondary-200 text-gray fs-7 text-center text-nowrap">
                庫存
              </th>
              <th className="bg-secondary-200 text-gray fs-7"></th>
            </tr>
          ...
          <tbody>
            {filteredProducts.length > 0 ? (
              filteredProducts.map(product => (
                <tr key={product.id}>
                  <td className="text-center text-nowrap fw-normal ">
                    {product.product_code || '無編號'}
                  </td>
                  <td>
                    <img
                      className="rounded-3"
                  ...
                  <td>
                    ...
                    {!product.is_enabled && (
                      <span className="bg-secondary text-primary rounded-2 p-1 fw-normal">
                        未上架
                      </span>
                    )}
                  </td>
                  <td className="text-center">
                    ...
                    <p className="text-gray text-decoration-line-through fw-normal">
                      NT${product.origin_price}
                    </p>
                  </td>
                  <td className="text-center fw-normal">
                    {product.product_stock || 3}
                  </td>
                  <td>
                    <div className="btn-group d-none d-md-block">
                      <button
                        type="button"
                        className="btn bg-transparent text-accent btn-sm"
                    ...
                    <div className="btn-group d-md-none d-flex flex-column">
                      <button
                        type="button"
                        className="btn bg-transparent text-accent btn-sm text-nowrap"
                        onClick={() => openModal('edit', product)}
                      >
                        編輯
                      </button>
              ...
              <tr>
                <td colSpan="6" className="text-primary text-center py-3">
                  目前沒有符合條件的商品
                </td>
              </tr>
            )}
