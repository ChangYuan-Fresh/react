      ...
      <div className="container px-0">
        ...
        >
          {orderState.map(state => {
            return (
              <SwiperSlide
                className={`nav-item  border-bottom ${selectState === state ? 'border-primary border-3' : 'border-gray-200'}`}
                key={state}
              >
                <button
                  className="nav-link px-3  border-0 py-3 w-100 text-center"
                  type="button"
            ...
            {orderState.map(state => {
              return (
                <li className="nav-item" key={state}>
                  <button
                    className={`nav-link bg-white border-0 py-4 text-custom ${selectState === state ? 'border-bottom border-3 border-primary text-primary' : ' border-0 text-dark'}`}
                    type="button"
                    onClick={() => setSelectState(state)}
                  >
                    {state}
          ...
          <div className="tab-content px-lg-5 px-0 bg-custom">
            <div className="tab-pane fade show active">
              ...
              <div className="input-group mb-3 w-100 w-lg-50 pt-lg-5 pt-3 d-none d-lg-flex">
                <select
                  className="form-select py-lg-3 py-0 pe-lg-4 pe-2 "
                ...
                <button
                  type="button"
                  className="btn border border-top border-bottom border-end"
                  onClick={handleSearch}
                >
                  <span className="material-symbols-outlined px-2 my-auto align-middle">
                    search
                  </span>
                </button>
              </div>
              <div className="mt-lg-6 mt-3 text-muted d-none d-lg-block">
                共{filterOrders.length}筆訂單
              </div>
              {/* 手機版搜尋列*/}
              <div className="input-group mb-3 w-100 w-lg-50 pt-lg-5 pt-3 d-lg-none bg-light mb-0 pb-0">
                <select
                  className="form-select py-lg-3 py-0 pe-lg-4 pe-2 bg-light"
                  style={{ maxWidth: '120px' }}
                  value={searchType}
                  onChange={e => setSearchType(e.target.value)}
                >
                ...
                <button
                  type="button"
                  className="btn border border-top border-bottom border-end bg-light"
                  onClick={handleSearch}
                >
                  <span className="material-symbols-outlined px-2 my-auto align-middle bg-light">
                    search
                  </span>
                </button>
              </div>
              <div className="mt-lg-6 mt-3 text-muted d-lg-none bg-light">
                共{filterOrders.length}筆訂單手
              </div>
                ...
                <table className="table mt-4 en-font  d-none d-lg-table">
                  <thead>
                    <tr>
                      <th
                        scope="col"
                        className="bg-secondary-200 text-muted p-3"
                      >
                        訂單編號
                      </th>
                      <th
                  ...
                  <tbody>
                    {filterOrders.length === 0 ? (
                      ...
                      filterOrders.map(order => {
                        const shippingFee = isNaN(Number(order.user?.shipping))
                          ? 0
                          : Number(order.user?.shipping);
                        const totalAmount = Math.floor(
                          (order.total ?? 0) + shippingFee
                        );
                        return (
                          <tr key={order.id} className="align-middle en-font">
                            ...
                            <td
                              className={`px-3 ${order.is_paid ? 'text-primary' : 'text-accent'}`}
                            >
                              {order.is_paid
                                ? order.orderStatus || '待出貨'
                                : '未付款'}
                            </td>
                            <td>
                              <button
          ...
          {filterOrders.length === 0 ? (
            ...
            filterOrders.map(order => {
              ...
              return (
                <>
                  <div
                    className="card border-0 bg-secondary-200 mt-3 d-lg-none"
                    key={order.id}
                  ...
                  >
                    <div className="card-title d-flex justify-content-between align-items-center border-bottom">
                      <button
                        type="button"
                        className="btn text-accent fs-5 ms-3"
                        onClick={() => openModal(order)}
                      >
                        {order.create_at}
                      </button>
                    ...
                    <div className="card-body">
                      <p className="mb-3">
                        <small className="text-gray me-3">訂購時間</small>
                        <FormatDate timestamp={order.create_at} />
                      </p>
                      <p className="mb-3">
                        <small className="text-gray me-3">訂購帳戶</small>{' '}
                        {order.user?.name}
                      </p>
                      <div className="d-flex justify-content-between align-items-center">
                        <p>
                          <small className="text-gray me-3">訂單金額</small> NT$
