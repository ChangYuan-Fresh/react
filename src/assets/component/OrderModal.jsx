  ...
  const btnUpdateOrder = async () => {
    try {
      const res = await axios.put(
        `${baseUrl}/v2/api/${apiPath}/admin/order/${tempOrder.id}`,
        {
          data: {
            ...tempOrder,
            ...
            <ul className="row list-unstyled">
              ...
              <li className="col-lg-4 mb-3">
                ...
                <input
                  ...
                  disabled
                  value={
                    tempOrder.create_at ? FormatDate(tempOrder.create_at) : ''
                  }
                />
              </li>
              <li className="col-lg-4 mb-3">
              ...
              <li className="col-12 mb-3">
                ...
                <input
                  ...
                  name="email"
                  disabled
                  value={`${tempOrder.user?.city ?? ''}${tempOrder.user?.district ?? ''}${tempOrder.user?.address ?? ''}`}
                />
              </li>
              <li className="col-12 mb-3">
              ...
              <li className="col-12 mt-5 bg-secondary-200 rounded-3">
                <table className="table bg-secondary-200 mt-5 table-borderless table-responsive">
                  <thead className="border-bottom">
                    <tr>
                      <th width="25%" className="bg-secondary-200">
                        品名
                      </th>
                      <th width="15%" className="bg-secondary-200">
                        單價
                      </th>
                      <th width="10%" className="bg-secondary-200">
                        數量
                      </th>
                      <th width="25%" className="bg-secondary-200">
                        折扣碼
                      </th>
                      <th width="20%" className="bg-secondary-200 text-end">
                        總價
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {tempOrder.products &&
                      Object.values(tempOrder.products).map(item => {
                        return (
                          <tr key={item.id}>
                            <th className="bg-secondary-200 py-3">
                              {item.product.title}
                            </th>
                            <td className="bg-secondary-200 py-3">
                            ...
                            <td className="bg-secondary-200 py-3">
                              {item.coupon?.code || '無優惠碼'}
                            </td>
                            <td className="bg-secondary-200 text-end py-3">
                              NT${item.total}
                            </td>
                          </tr>
                        );
                  ...
                  <tfoot className="border-top">
                    <tr>
                      ...
                      <td className="bg-secondary-200 d-lg-flex align-items-center mt-3 d-none">
                        <label
                          htmlFor="shipping"
                          className="form-label text-nowrap me-2 mb-0"
                        >
                          運費
                        </label>
                        <input
                      ...
                      <td className="bg-secondary-200">
                        <div className="text-nowrap d-lg-flex justify-content-end mt-3 d-none">
                          總計
                          <span className="bg-secondary-200 ms-2">
                            NT$
                            {Math.floor(tempOrder.total) +
                              (isNaN(Number(tempOrder.user?.shipping))
                                ? 0
                                : Number(tempOrder.user?.shipping))}
                          </span>
                        </div>
                      </td>
                    </tr>
              ...
              <li className="col-lg-6 form-check ms-auto mt-5 px-0">
                <div className="d-flex mb-3 d-lg-none">
                  ...
                  <div className="text-nowrap d-flex justify-content-end mt-3">
                    總計
                    <span className="ms-2">
                      NT$
                      {Math.floor(tempOrder.total) +
...
OrderModal.propTypes = {
  modelRef: PropTypes.shape({ current: PropTypes.instanceOf(Element) })
    .isRequired,
