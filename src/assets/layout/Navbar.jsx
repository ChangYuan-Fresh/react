      ...
      <header className="navbar navbar-expand-lg p-0 bg-white sticky-top">
        <div className="container-fluid py-4 px-3">
          <NavLink className="navbar-brand" to="/">
            <picture>
              <source media="(min-width: 992px)" srcSet={logoL} />
              <img src={logoS} alt="logo" />
            </picture>
          </NavLink>
          <NavLink
            className="fs-2 text-primary ms-auto me-3 d-lg-none position-relative"
            to="cart"
          >
            <span className="material-symbols-outlined">shopping_cart</span>
            {cartNum?.length > 0 && (
              <span
                className="badge rounded-pill text-bg-danger position-absolute top-0 start-100 translate-middle p-0 d-flex align-items-center justify-content-center mt-3"
                style={{ width: '14px', height: '14px', fontSize: '10px' }}
              >
                {cartNum?.length}
              </span>
            )}
          ...
          >
            {isOpen ? (
              <span className="material-symbols-outlined border-0">close</span>
            ) : (
              <span className="material-symbols-outlined border-0">menu</span>
            )}
          </button>
          <div
          ...
          >
            <div className="navbar-nav ms-auto mt-5 mt-lg-0">
              <NavLink
                className="nav-link text-center py-6 py-lg-0 px-lg-6 border-fix"
                to="about"
              ...
              <NavLink
                className="nav-link text-center py-6 py-lg-0 px-lg-6"
                to="faq"
                onClick={handleLinkClick}
              >
              ...
              <NavLink
                className="btn btn-s d-none d-lg-inline-block p-0 border-0 mx-6"
                to="cart"
              >
                <button type="button" className="btn position-relative p-0">
                  ...
                  {cartNum?.length > 0 && (
                    <span
                      className="badge rounded-pill text-bg-danger position-absolute top-0 start-100 translate-middle fs-6 d-flex justify-content-center align-items-center"
              ...
              <NavLink
                className="btn btn-s d-none d-lg-inline-block p-0 border-0"
                to="login"
              >
                <span className="material-symbols-outlined p-4 fs-2">
                  person
              ...
              <div className="d-lg-none">
                <NavLink
                  className="nav-link py-6 link-primary text-center py-6"
                  to="/login"
                  onClick={handleLinkClick}
                >
